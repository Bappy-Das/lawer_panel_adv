<template>
  <div
    :class="['date-picker-wrapper', { 'date-picker-wrapper-reset': reset === false || disabled || readonly }, { 'date-picker-wrapper-disabled': disabled }, { 'date-picker-wrapper-readonly': readonly }]">
    <Datepicker v-model="datePicker" ref="datepickerRef" :menuClassName="datepickerClass" :range="range"
      :multiCalendars="multiCalendars" :autoApply="applyButton == 'default' ? true : false" :offset="0"
      :placeholder="placeholder" :position="position" :multiCalendarsSolo="multiCalendarsSolo" :enableTimePicker="false"
      :transitions="false" :maxDate="maxDate" :minDate="minDate" :presetRanges="presetRanges" :format="format"
      :key="datePickerId + '-' + key" @update:modelValue="handleDate" @open="openMenu" />
    <!-- :transitions="false" -->
    <input class="date-range" type="hidden" v-model="modelValueTemp" :name="name" />
...
watchEffect(() => {
  if (props.range === false) {
    if (props.modelValue == (null || '' || undefined)) {
    ...
    if (validVar(props.modelValue)) {
      ...
      let startDate, endDate;
      if (dateArr[0]) { startDate = new Date(dateArr[0]) } else { startDate = 'Invalid Date' };
      if (startDate == 'Invalid Date') { startDate = null }
      if (dateArr[1]) { endDate = new Date(dateArr[1]) } else { endDate = 'Invalid Date' };
