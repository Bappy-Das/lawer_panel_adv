<template>
  <PageLayout :header="useRoute().meta.pageTitle">
    <h4 class="mb-15">
      Example With Simple Array ['Cat', 'Dog', 'Tiget', 'Lion']
    </h4>
    <div
      class="cursor-pointer pa-10 bg-primary text-white no-text-select d-inline-block mb-20"
      @click="changeGenValue"
    >
      Change Value
    </div>
    <div
      class="cursor-pointer pa-10 bg-primary text-white no-text-select d-inline-block mb-20 ml-10"
    ...
    <div style="max-width: 200px">
      <SelectBox
        ...
        resetVal
        v-model="selectmoedelGen"
        reset
        @reset="resetData"
        placeholder="PlaceHolder"
        name="selectbox"
        :options="['Cat', 'Dog', 'Tiget', 'Lion']"
        :showDetail="true"
      >
    ...
    <h4 class="mb-15">
      Example With Multidimantional Array [{id,name},{id,name}]
    </h4>
    <div
      @click="changeValue()"
      class="cursor-pointer pa-10 bg-primary text-white no-text-select d-inline-block mb-20"
    ...
    <h4 class="mb-15">Example Of slots x</h4>
    <div style="max-width: 200px">
      <SelectBox
        :validate="Yup.string().required()"
        @optionSearch="slotSearchText = $event"
        :inputSearch="true"
        resetVal
        @reset="resetData"
        placeholder="PlaceHolder"
        ...
        name="selectslot"
        :options="slotSearchOption"
        :showDetail="true"
      >
        <template v-slot:input="{ data }">
          <div class="d-flex-v-center px-10">
            <div
              class="round-image bg-background mr-10"
              style="width: 30px; height: 30px; background-color: red;"
            ></div>
            <div>{{ data.name }} xxxx</div>
          </div>
        </template>
        <template v-slot:option="{ data }">
          <div class="d-flex-v-center px-10 py-5">
            <div
              class="round-image bg-background mr-10 bg-primary-shade"
    ...
    <h4 class="mb-15">
      Example Of multiselect checkbox and option header search
    </h4>
    <div style="max-width: 200px">
      <SelectBox
        :validate="Yup.string().required()"
        v-model="stCheckedString"
        disableOptionClick
        disableOnClickOptionClose
        resetVal
        @reset="resetData"
        placeholder="PlaceHolder"
        show="name"
      ...
      >
        ...
        <template v-slot:option="{ data }">
          <div class="d-flex-v-center pa-10 py-5">
            <div class="d-flex-v-center mr-10">
    ...
    <h4 class="mb-15">Example Of large input</h4>
    <div style="max-width: 200px">
      <SelectBox
        size="lg"
        :inputSearch="true"
    ...
    <div>[input, option] For custom input and option preview</div>
    ...
    <div>
      ...
      <strong>size</strong> [Options:sm]<br />
      <strong>disabled</strong><br />
      <strong>readonly</strong><br />
      <strong>resetVal</strong> [if true, reset value icon will show]<br />
      <strong>autocomplete</strong> [Enable disable browser autocomplete for
      input search]<br />
      <strong>options</strong> [Could be array or array objec. if object, then
      show and catch props is required]<br />
      <strong>show</strong> [if option is multidimantional array, Object's field
      name that will be shown]<br />
      <strong>catch</strong> [if option is multidimantional array, Object's
      field that will return in v-model]<br />
      <strong>inputSearch</strong> [If true, on click an input field will
      appear]<br />
      <strong>showDetail</strong> [Maintain space for error, success, warning
      msg | Default value is false]<br />
      <strong>message</strong> [message like error or warning]<br />
      <strong>messageType</strong> [message type Ex:error, warning, success, no
      value will show plain test]<br />
    </div>
