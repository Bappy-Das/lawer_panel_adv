<template>
  <PageLayout header="Delivery Invoice">
    ...
    <div v-else id="invoicePage" class="d-flex-center" v-for="(item, index) in deliveryStore.selectActionItem"
      :key="'main-' + index">
      <div class="mb-20">
        <div class="mt-30 ">
          <svg width="140" height="80" viewBox="0 0 279 110" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_512_4549)">
              ...
              <path d="M227.456 26.7316V39.5524H245.132V47.2545H261.409V39.5524H279V26.7316H227.456Z" fill="#333333" />
        ...
        <div class="my-20">
          ...
          <div v-if="item.merchant_info.business_name" class="text-gray-2 text-4-14">{{ item.merchant_info.business_name
          }}</div>
          <div class="text-gray-2 text-4-14">{{ item.merchant_info.first_name + " " + item.merchant_info.last_name }}
          </div>
          <div v-if="item.merchant_info.operator_number" class="text-gray-2 text-4-14">{{
            item.merchant_info.operator_number }}</div>
        </div>
        <div class="mb-20">
          <div class="text-5-18">Shipped To : </div>
          <div v-if="item.recipient_name" class="text-gray-2 text-4-14">{{ item.recipient_name }}</div>
          <div v-if="item.recipient_number" class="text-gray-2 text-bold">{{ item.recipient_number }}</div>
          <div v-if="item.recipient_email" class="text-gray-2 text-4-14">{{ item.recipient_email }}</div>
          <div v-if="item.recipient_address" class="text-gray-2 text-4-14" style="width:200px">{{ item.recipient_address
          }}</div>
        </div>
        <div class="mb-20">
          <div class="text-5-18">Order summary : </div>
          <div v-if="item.product_name" class="text-gray-2 text-4-14">Item: {{ item.product_name }}</div>
          <div v-if="item.merchant_order_id" class="text-gray-2 text-4-14">Merchant Order ID: {{ item.merchant_order_id }}
          </div>
          <div v-if="item.created_at" class="text-gray-2 text-4-14">Order Date:
            {{ item?.created_at?.replace("/", "-").replace("/", "-") }}</div>
          <div v-if="item.amount_to_be_collected" class="text-gray-2 text-4-14">Total Receivable:
            <span class="text-bold">{{ item.amount_to_be_collected }} Tk</span>
          </div>
        </div>
        <div v-if="item.special_instruction" class="mb-20">
          <div class="text-5-18">Special Instruction:</div>
          <div class="text-gray-2 text-4-14">{{ item.special_instruction }}</div>
        </div>
        <div class="mb-20">
          <canvas ref="barcodeCanvas" :id="'barcode-' + index"></canvas>
          <div class="text-gray-2 text-4-14 ml-45 pa-5">{{ item.consignment_id }}</div>
        </div>
        <hr class="hr-line">
      </div>
...
// console.log(deliveryStore.selectActionItem)


// Function to generate barcode
const generateBarcode = (canvasId, consignmentId) => {
  JsBarcode(`#${canvasId}`, consignmentId, {
    format: 'CODE128',
    displayValue: false,
...
onMounted(() => {
  const items = deliveryStore.selectActionItem;
  items.forEach((item, index) => {
    const canvasId = `barcode-${index}`;
    generateBarcode(canvasId, item.consignment_id);
  });
});
...
<html>
  ...
  // let data = document.getElementById('invoicePage');
  // window.print(data);
  // body = body;


  // window.print(document.getElementById("invoicePage"));
  // const win = window.open(document.getElementById("invoicePage"));
  // win.document.write(document.getElementById("invoicePage"))
  // win.document.close();
  // win.focus();
  // win.print();
  // win.close();
