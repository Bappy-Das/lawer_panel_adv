<template>
    <div class="result-per-page--container">
        ...
        <div>
            <div class="result-per-page--per-page" ref="mainContainerRef">{{ perPage }}</div>
            <div ref="mainOptionContainerRef">
                <transition :name="dropdownAnimation">
                    <div class="result-per-page--option-container thin-scrollbar" v-if="toggle" ref="optionContainerRef"
                        :style="{ maxHeight: dropdownHeight + 'px' }">
                        <div class="result-per-page--option-item" v-for="(item, index) in pageNumberOptions"
                            :key="index" @click="loadItem(item)">{{ item }}</div>
                    </div>
                </transition>
            </div>
        </div>
