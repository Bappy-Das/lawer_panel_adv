<template>
    ...
    <div :class="['pagination--container', { 'pagination--container-block': block }]">
        <div :class="['pagination--btn-nav noselect', { 'pagination--btn-nav-container-disable': !navs.prev.active }]"
            @click="loadPage(navs.prev.page)">
            <span :class="['fa-solid fa-chevron-left', { 'pagination--btn-nav-disable': !navs.prev.active }]"></span>
        </div>
        <div v-for="(item, index) in pages" :key="index"
            :class="['pagination--btn-page noselect', { 'pagination--btn-page-active': item.active }, { 'pagination--btn-page-disable': item.active || item.page == '--' }]"
            @click="loadPage(item.page)">{{ item.page }}</div>
        <div :class="['pagination--btn-nav noselect', { 'pagination--btn-nav-container-disable': !navs.next.active }]"
            @click="loadPage(navs.next.page)">
            <span :class="['fa-solid fa-chevron-right', { 'pagination--btn-nav-disable': !navs.next.active }]"></span>
        </div>
    </div>
</template>
...
const caclPages = () => {
    ...
    if (prevPages < 1 && (totalPage - nextPages) < 0) {
        ...
        if ((nextPages + Math.abs(prevPages)) > totalPage) {
            ...
            // console.log(nextPages, Math.abs(prevPages))
...
watchEffect(() => {
    if (!validNumber(props.currentPage) || !validNumber(props.totalPage) || validNumber(props.currentPage, 0) > validNumber(props.totalPage, 0)) {
        //console.log(!validNumber(props.currentPage), !validNumber(props.totalPage), validNumber(props.currentPage, 0) > validNumber(props.totalPage, 0), validNumber(props.currentPage, 0), validNumber(props.totalPage, 0))
        navs.next = { active: false, page: null };
        navs.prev = { active: false, page: null };
        pages.value = [];
        return
